...
def build():
    ...
    try:
        ...
        # Run PyInstaller with explicit output paths
        subprocess.run([
            pyinstaller_path,
            '--noconfirm',
            '--onefile',
            '--noconsole',
            '--distpath', str(releases_dir),  # Specify output directory
        ...
        exe_path = releases_dir / 'WebDesk.exe'
        if not exe_path.exists():
            raise RuntimeError(f"Failed to create executable at {exe_path}")
